...
class mint_page(mint_pageTemplate):
  ...
  def refresh_mint(self):
    b = self.team_contract.balanceOf(self.address)
    
    
    self.team_balance = int(self.team_contract.balanceOf(self.address).toString()) if b._isBigNumber else int(self.team_contract.balanceOf(self.address).toNumber())
    ...
    days_remaining = self.minting_phase_end_day-self.hex_day
    if days_remaining<0:
      self.column_panel_3.clear()
      self.column_panel_3.add_component(Label(text="TEAM Minting is Now Complete"))
      
    else:
      self.label_mint_duration.text= '{} Days Remaining'.format(days_remaining)
      ...
      "%m/%d/%Y, %H:%M:%S"
      print(self.hex_day)
      print(self.minting_phase_end_day)
  ...
  def check_approval(self):
    ...
    self.approved_maxi = int(self.maxi_contract.allowance(self.main.address, self.main.web3_wallet.TEAM_CONTRACT_ADDRESS).toString() or 0)
